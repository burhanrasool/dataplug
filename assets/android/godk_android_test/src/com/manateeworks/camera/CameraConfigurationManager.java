...
final class CameraConfigurationManager {
	...
	public static Point getMaxResolution(Camera.Parameters parameters) {
		...
		return new Point(sizes.get(minIndex).width, sizes.get(minIndex).height);
	}
	...
	private static Point getCameraResolution(Camera.Parameters parameters, Point screenResolution) {
		...
		for (int i = 0; i < sizes.size(); i++) {
			int dif = Math.abs(sizes.get(i).width - CameraManager.mDesiredWidth) + Math.abs(sizes.get(i).height - CameraManager.mDesiredHeight);
			if (dif < minDif) {
				minDif = dif;
				minIndex = i;
			}
	...
	// // And this is a hack-hack to work around a different value on the Behold
	// II
	...
	// parameters.get("taking-picture-zoom-max");
