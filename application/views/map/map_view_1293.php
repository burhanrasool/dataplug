...
//        markerClusterer.clearMarkers();
    ...
    function addMarker() {
        ...
        var form_list = $(".form_list").multipleSelect('getSelects');
//        alert(selected_form_id.length);
        if (form_list.length == 0) {
            var selected_form_id = '<?php echo $form_id; ?>';
        } else {
            var selected_form_id = form_list;
        }
    ...
    function toggleMarkersShow(markerCategory) {
        ...
        if (counter <= 1) {
            $('.overlay .message').text(counter + ' Record ');
        } else {
...
<div id="container">
    <div class="inner-wrap" style="z-index:999; position: relative;">
        ...
        <?php
        $apps_name = str_replace(' ', '-', $app_name);
        ...
        <div class="filter_class">
            ...
            if ($district_filter == "On") {
                ...
                <div class="filter-div-map">
                    Districts :  
                    <select  name="district_ajax" id="district_ajax" onchange="getDistrictBasedTown(this)"  style="width:81px;" >
                        <option selected="selected" value="">Select District</option>
                        <?php
                        foreach ($district_list as $district) {
                            if (isset($district['district_name']) && $district['district_name'] != '') {
                                ?>
                                <option value="<?php echo strip_tags($district['district_name']); ?>"><?php echo $district['district_name']; ?></option>
                                <?php
                            }
                        }
                        ?>
...
<!--                    <select class="required customSelect" name="filters" id="filter" style="width:188px" onChange="$('#overlay_loading').show();
                            filter_update('<?php echo $app_id ?>', $(this).val())"/></select>-->
                                        ...
                                        foreach ($category as $cat) {
                                            if (!in_array($cat, $category_included)) {
                                                if (!in_array($form_item[$filter_attribute_value], $only_once_category)) {
                                                    ...
                                                    $value_attribute = html_entity_decode($cat);
                                                    $value_attribute = str_replace('&', 'and', $value_attribute);
                                                    ?>
                                                    <div style="float: left; width: 100%; margin-top: 2px;">
                                                        <p>
                                                            <input type="checkbox" id="filter_check"  class="filter checkbox category public" name="check_box_filter[]" value="<?= $value_attribute; ?>">
    ...
    <div style="position: absolute; bottom: 15px;margin-left:28%;z-index: 999;font-weight: bold;">All Rights Reserved © 2013-<?php echo date('Y');?> - DataPlug By ITU Government of Punjab - Pakistan.</div>
</div>
...
<script type="text/javascript">
    ...
    function getDistrictBasedTown()
    {
        /* Getting Towns based on district logict will be here */
    }
    jQuery(document).ready(function() {
        ...
        /**
         * FOR District aJAX cALL
         */

        $('#district_ajax').change(function() {
            clearOverlay();
//            clusterView(1);
            $('#overlay_loading').show();

            var district_filter_exist = jQuery('#district_ajax :selected').val();

            if (typeof district_filter_exist != 'undefined') {
                var district_filter = jQuery('#district_ajax :selected').val();
            } else {
                var district_filter = "";
            }
//            alert(town_filter);
            if (district_filter == "") {
                ...
                var datum = 'district=' + district_filter;
                $.ajax({
                    dataType: 'json',
                    url: "<?= base_url() . 'form/district_wise_record?&app_id=' . $app_id; ?>",
                    data: datum,
                    success: function(resp) {
        ...
        $('input.category').change(function() {
            ...
            if (selected_view == 'Heat') {
                ...
                if (!$(this).is(':checked')) {
                    counter = 0;
                    for (var i = 0; i < markers.length; i++) {
                        var comparerss = markerCategory.trim();
                        var title_list = markers[i].title.trim();
                        var res = title_list.match(comparerss);
                        if (res) {
                            counter = counter + 1;
                    ...
                    for (var i = 0; i < markers.length; i++) {
                        var title_list = markers[i].title;
                        var res = title_list.match(markerCategory);
                        if (res) {
