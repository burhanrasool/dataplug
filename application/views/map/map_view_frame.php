...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var zoom_level_db = <?php echo $zoom_level; ?>;
        var latitude_db = <?php echo $latitude; ?>;
        var longitude_db = <?php echo $longitude; ?>;
        ...
        for (i = 0; i < locations.length; i++) {
            ...
            google.maps.event.addListener(companyMarker, 'click', (function(companyMarker, i) {
                return function() {
                    $.ajax({
                        url: '<?= base_url() ?>form/map_activity_popup?form_result_id=' + locations[i][4],
...
//        });
    ...
    function addMarker() {
        ...
        var page = temp[1];
        var datum = 'filter_date_to=' + filter_date_to + '&page=' + page + '& filter_date_from=' + filter_date_from + '& town_filter=' + town_filter;
        $.ajax({
            dataType: 'json',
            url: "<?= base_url() . 'form/moreMarker?&form_id=' . $form_id; ?>",
            data: datum,
            success: function(resp) {
                ...
                $.each(resp, function(i) {
                    ...
                    google.maps.event.addListener(companyMarker, 'click', (function(companyMarker, i) {
                        return function() {
                            $.ajax({
                                url: '<?= base_url() ?>form/map_activity_popup?form_result_id=' + resp[i].form_result_id,
                                success: function(data) {
                                    infowindow.setContent(data);
                                    infowindow.open(map, companyMarker);
                                }
    ...
    function toggleMarkersShow(markerCategory) {
        ...
        $('.overlay .message').text(counter + ' Records ');
        setTimeout(function() {
...
<div id="form_descrip" style="display:none;"><?php echo $form_html; ?></div>

<div class="inner-wrap">
    <h5 style="text-transform: capitalize; z-index: 10px; margin-bottom: -32px">
        <div class="table-sec"> 
            ...
            <div class="filter_class">
                ...
                echo form_open(base_url() . 'form/mapviewframe/' . $app_id, 'id=date_filter_form name=date_filter_form');
...
//                    }
                ...
                <?php } ?>
                <?php
                echo '<div class="form_class" style="float: left;margin-left:8px">';
                ...
                <div class="filter-div-map" style="margin-left:5px">
                    <input type="hidden" value="<?php echo $form_id; ?>" name="form_id">
                    From : 
                    <input type="text" size="7" id="datepicker" value="<?php echo!empty($selected_date_to) ? $selected_date_to : ''; ?>" name="filter_date_to" onchange="check_date_validity()" ondblclick="clear_field(this)">
                    To :
                    <input type="text" size="7" id="datepicker2" value="<?php echo!empty($selected_date_from) ? $selected_date_from : ''; ?>" name="filter_date_from" onchange="check_date_validity()" ondblclick="clear_field(this)">
                ...
                <?php echo form_close(); ?>
                <!--Filter Ends here-->
                <div  style="margin:6px 0 0px 916px;position:fixed">
                    <span id="counter_div" >
                        <?php echo count($form); ?>
                    </span>
                    Records 
                </div>
                <!--                    <div  style="margin:0 0 1px 1000px;position:fixed">
                <?php if ($totalPages > 1) { ?>
            ...
            <!--<div id="map"></div>-->
            <div id="map" style="position: relative !important; height:620px;">
            ...
            <div class="CrimeList">
                <!--                                    <div class="crimeListTexta" style="width:auto;position: absolute;top: -35px;">
                                                        <form id='setfilter' method='POST' action='<?= base_url() ?>form/changefilter/<?php echo $form_id ?>'>
                                                            ...
                                                            Change Filter 
                                                            <select class="required" name="filter" id="filter" style="width:188px" onChange="$('#overlay_loading').show();
                                                                    $('#setfilter').submit();"/></select>
                                                        </form>
                                                    </div>-->
                <div>
                    <?php echo form_open('form/mapviewframe/' . $app_id, array('id' => 'checkbox_filter_form', 'name' => 'cate_check_filter')); ?>
                    <div style="float: left; width: 100%; background-color: #818E90; position: fixed; margin-bottom: 30px;">
                        <p style="height: 30px;line-height: 25px;">
                            <input type="checkbox" style="margin-top: 8px;margin-left: 10px;" checked="checked" id="check_all">
                            <label>
                                <b style="font-size: 13px; color: white;">All Categoies</b>
                            </label>
                        </p>
                    </div>
                                                    ...
                                                    <p><?php
                                                        if (isset($selected_check_boxes)) {
                                                            if (in_array($form_item[$filter_attribute_value], $selected_check_boxes)) {
                                                                ?>
                                                                <input type="checkbox" id="filter_check" checked="checked" class="filter checkbox category public" name="check_box_filter[]" value="<?= $form_item[$filter_attribute_value]; ?>">
                                                            <?php } else {
                                                                ?>
                                                                <input type="checkbox" id="filter_check"  class="filter checkbox category public" name="check_box_filter[]" value="<?= $form_item[$filter_attribute_value]; ?>">
                                                                <?php
                                                            }
                                                        } else {
                                                            ?>
                                                            <input type="checkbox" id="filter_check" checked="0" class="filter checkbox category public" name="check_box_filter[]" value="<?= $form_item[$filter_attribute_value]; ?>"><?php
                                                        }
                    ...
                    <div class="crimeListText new" style="margin-top:0px">

                        <div style="float: left; width: 100%; background-color: #818E90;">
                            <p style="height: 24px;">
                                <label>
                                    <b style="font-size: 13px; color: white;">Cluster Labels</b>
                                </label>
                            </p>
                        </div>
                        <div id="cluster_label" style="display:block;">
                            <img width="25px" height="20px" src="<?php echo base_url() . "assets/images/blue_cluster.png"; ?>">
                            <label>1-10 Records</label>
                        </div>
                        <div id="cluster_label" style="display:block;">
                            <img width="25px" height="20px" src="<?php echo base_url() . "assets/images/yellow_cluster.png"; ?>">
                            <label>10-100 Records</label>
                        </div>
                        <div id="cluster_label" style="display:block;">
                            <img width="25px" height="20px" src="<?php echo base_url() . "assets/images/red_cluster.png"; ?>">
                            <label>100-1000 Records</label>
                        </div>
                        <div id="cluster_label" style="display:block;">
                            <img width="25px" height="20px" src="<?php echo base_url() . "assets/images/pink_cluster.png"; ?>">
                            <label>1000-Above Records</label>
                        </div>
                    </div>
                </div>
            ...
            <script type="text/javascript">
                ...
                function getTownBasedUc()
                {
                    ...
                    if (town_filter == "") {
                        ...
                        $.ajax({
                            dataType: 'json',
                            url: "<?= base_url() . 'form/town_wise_uc?&form_id=' . $form_id; ?>",
                            data: datum,
                            success: function(resp) {
                ...
                jQuery(document).ready(function() {
                    ...
                    $('#town').change(function() {
                        ...
                        if (town_filter == "") {
                            ...
                            $.ajax({
                                dataType: 'json',
                                url: "<?= base_url() . 'form/town_wise_record?&form_id=' . $form_id; ?>",
                                data: datum,
                                success: function(resp) {
                    ...
                    $('#uc').change(function() {
                        ...
                        if (uc_filter == "") {
                            ...
                            var datum = 'uc=' + uc_filter;
                            $.ajax({
                                dataType: 'json',
                                url: "<?= base_url() . 'form/uc_wise_record?&form_id=' . $form_id; ?>",
                                data: datum,
                                success: function(resp) {
                    ...
                    $('input.category').change(function() {
                        //            $('select#view_type').val('Pin').trigger('change');
...
//                    jQuery('.gm-style-iw').next().click();
                ...
                $('#form-preview').find('input, textarea, select').each(function() {
                    ...
                    if ($.inArray(field_name, templist) == '-1')
                    {
                        ...
                        if (field_name != 'form_id' && field_name != 'takepic' && skip != 'skip' && type != 'submit')
                        {
            ...
            <style>
                ...
                .dropDownDiv {
                    background: none repeat scroll 0 0 #818E90;
                    display: block;
                    margin-top: 4px;
                    padding: 3px 0;
                }
