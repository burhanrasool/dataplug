...
echo '<div class="form_class" style="' . $style . '">';
...
<form id='setfilter' method='POST' action='<?= base_url() ?>graph/dashboard/<?php echo $app_id ?>'>
    ...
    <input name="custom1567" value="app1567" type="hidden" />
    <input name="customform" id="custom_form" value="" type="hidden" />
    <input name="customdate" id="custom_date" value="" type="hidden" />
    Change Category :
<?php
...
if ($app_id == 1293) {
    ...
    <?php } else { ?>
    <select class="required" name="filter" id="filter1" style="width:188px" onchange="$('#custom_form').val($('#filter1 option:selected').attr('form_id'));
                $('#setfilter').submit();"/>
    <option display_value="Facilities Available" value="Facilities_Available" form_id="1654" <?php if ($filter == 'Facilities_Available') {
        echo 'selected';
    } ?>>Facilities Available</option>
    <option display_value="Type" value="Type" form_id="1768" <?php if ($filter == 'Type') {
        echo 'selected';
    } ?>>Incident Type</option>
    <option display_value="Designation" value="Designation" form_id="1769" <?php if ($filter == 'Designation') {
        echo 'selected';
    } ?>>Visit of officials</option>
    <option display_value="Satisfied_from_the_service" value="Satisfied_from_the_service" form_id="1770" <?php if ($filter == 'Satisfied_from_the_service') {
        echo 'selected';
    } ?>>Satisfied from the service</option>
    </select>
<?php } ?>
</form>
...
<div class="applicationText">
    ...
    <div id="container_pie" style="width: 100%; height: 441px; overflow: auto; margin: 0 auto">
     ...
     if(isset($disbursement_type_rec) && $disbursement_type_rec){
        ?>
    <div id="container_stack" style="width: 100%; height: 441px; overflow: auto; margin: 0 auto">
    <?php }?>
    </div>
    <hr>
<?php
if ($app_id == '1567') {
    if(isset($form_id) && $form_id=='1654'){
    ?>
        <h1>
                Facilities Availability Report
                </h1>
                <label style="padding-right:19px;">Date</label> : 
                <input size ="15" type="text" id="datepicker" readonly="readonly" value="<?php echo $date_search;  ?>" onchange="$('#custom_date').val($(this).val());$('#custom_form').val('1654');$('#setfilter').submit();">
    <?php }
    if (isset($disbursement_rec) && $disbursement_rec) {
        ?>
    ...
    <?php }
            ...
            foreach ($category_list_count as $key => $counts) {
                ...
                $list_x_axix .= "'".preg_replace('/[^A-Za-z0-9() -]/', '', $key)."'" . ',';
                $list_y_axix .= "$counts" . ',';
            }
            ...
            foreach ($pie_array as $key => $counts) {
                $pie_chart_data .= '[' . "'".preg_replace('/[^A-Za-z0-9() -]/', '',$key)."'" . ',' . $counts . '],';
            }
            /*
